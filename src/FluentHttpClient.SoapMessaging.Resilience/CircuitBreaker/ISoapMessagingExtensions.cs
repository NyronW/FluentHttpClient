...
{
    public static async Task<HttpResponseMessage> SoapPostAsync(this ISendRequestWithBody client, string soapPayload, AsyncCircuitBreakerPolicy policy)
    {
        if (policy != null)
    ...
    public static async Task<HttpResponseMessage> SoapPostAsync<TRequest>(
        this ISendRequestWithBody client, TRequest request, AsyncCircuitBreakerPolicy policy, string methodName = "",
